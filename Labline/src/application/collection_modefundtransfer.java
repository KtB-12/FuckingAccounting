...
public class collection_modefundtransfer {
    ...
    @FXML private TextField referencenum;
    ...
    private void add_to_list(MouseEvent event) {
        ...
        if (amount == null || amount.isEmpty()) {
            ...
            PaymentRecord record = PaymentRecord.createFundTransfer("Fund Transfer", amount, transferDate, selectedBank, referenceNumber);
            PaymentSessionData.getInstance().addPaymentRecord(record);
            System.out.println("Fund transfer payment saved: " + amount);
            showPrompt("Payment Added", "Fund transfer payment saved successfully.");
            closeWindow(event);
        }
    }
